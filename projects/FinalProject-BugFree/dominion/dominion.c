...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
        cardDrawn = state->hand[currentPlayer][choice1];  //store card we will trash

        // Error: Selected card is not a treasure card
        if (cardDrawn < copper || cardDrawn > gold)
            return -1;
        // Error: Selected card is not a valid card
        if (choice2 > treasure_map || choice2 < curse)
            return -1;
        // Error: Selected card is too expensive
        if ( getCost(choice2) > getCost(cardDrawn) + 3 )
            return -1;
        ...
        for (i = 0; i < state->handCount[currentPlayer]; i++) {
            if (state->hand[currentPlayer][i] == cardDrawn) {
    ...
    case ambassador:
        cardDrawn = state->hand[currentPlayer][choice1];
        j = 0;

        // Error: Player must return 1-2 cards
        if (choice2 > 2 || choice2 < 0)
            return -1;
        // Error: Player cannot return Ambassador card being played
        if (choice1 == handPos)
            return -1;

        for (i = 0; i < state->handCount[currentPlayer]; i++)
            if (i != handPos && state->hand[currentPlayer][i] == cardDrawn)
                j++;

        // Error: Player does not have enough of this card type to return
        if (j < choice2)
            return -1;

        //increase supply count for choosen card by amount being discarded
        state->supplyCount[cardDrawn] += choice2;

        //each other player gains a copy of revealed card
        for (i = 0; i < state->numPlayers; i++)
            if (i != currentPlayer)
                gainCard(cardDrawn, state, 0, i);

        //discard played card from hand
        discardCard(handPos, currentPlayer, state, 0);

        // Trash cards that we returned to the supply. Iterate in reverse since
        // the discardCard function adjusts the player's hand each time it's called.
        for (i = state->handCount[currentPlayer], j = choice2; i >= 0 && j > 0; i--) {
            if (state->hand[currentPlayer][i] == cardDrawn) {
