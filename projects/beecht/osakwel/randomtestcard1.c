...
int main() {
    ...
    for (int test = 0; test < TEST_COUNT; test++) {
        int choice1 = rand() % 2;
        int handPos = locate_card(baron, state->whoseTurn, state);
        ...
        int ret = baronCardEffect(state, choice1, state->whoseTurn, handPos);
    ...
    assert_random("it never affects another player's state",
            results[0]);
    assert_random("it never affects the player's deck",
            results[1]);
    assert_random("it never increases the player's deck by more than 2 cards",
            results[2]);
    assert_random("it discards the Baron after playing it",
            results[3]);
    assert_random("it does not affect the player's number of actions",
            results[4]);
    assert_random("it gives the player +1 buys",
            results[5]);
    assert_random("it causes the player to gain an Estate if they do not discard an Estate",
            results[6]);
    assert_random("it doesn't change the number of Estates in hand if player does not discard",
            results[7]);
    assert_random("it causes the player to gain +4 coin if discarding an Estate",
            results[8]);
    assert_random("it doesn't affect the player's coins when not discarding an Estate",
            results[9]);
    assert_random("it only discards one Estate if the player has one in hand and discards",
            results[10]);
    assert_random("it fails if the player has no Estates and tries to discard one",
            results[11]);
    assert_random("it fails if the player does not have a Baron card",
            results[12]);
    assert_random("it succeeds when player does not discard or chooses to discard and has an Estate",
            results[13]);
