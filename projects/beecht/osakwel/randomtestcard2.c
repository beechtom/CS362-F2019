...
int main() {
    ...
    for (int test = 0; test < TEST_COUNT; test++) {
        ...
        int cardPos = locate_card(minion, after->whoseTurn, after);

        // Run the card effect with ~random~ input
        int ret = minionCardEffect(after, cardPos, after->whoseTurn, choice1, choice2);
    ...
    assert_random("it fails if the player does not have a Minion card",
            results[0]);
    assert_random("it discards the Minion card",
            results[1]);
    assert_random("it grants the player +1 action",
            results[2]);
    assert_random("it fails if the player tries to select both options",
            results[3]);
    describe("\tand the player chooses +2 coins");
    assert_random("\tit gives the player +2 coins",
            results[4]);
    assert_random("\tit does not make players draw cards",
            results[5]);
    describe("\tand the player chooses to discard");
    assert_random("\tit discards the player's hand",
            results[6]);
    assert_random("\tit gives the player +4 cards",
            results[7]);
    assert_random("\tit draws cards from the player's deck",
            results[8]);
    assert_random("\tit doesn't give the player +2 coins",
            results[9]);
    assert_random("\tit discards the hand of any player with 5 or more cards",
            results[10]);
    assert_random("\tit gives +4 cards to any player that had 5 or more cards",
            results[11]);
    assert_random("\tit draws cards from the deck of any player that had 5 or more cards",
            results[12]);
    assert_random("it fails when trying to discard something other than Minion",
            results[13]);
    assert_random("it succeeds when choosing one of the two choices and correctly indicating the Minion",
            results[14]);
